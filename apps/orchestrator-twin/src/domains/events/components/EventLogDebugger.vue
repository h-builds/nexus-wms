...
const isDebuggerPanelOpen = ref(false)
const selectedDebuggerTab = ref<'ingestion' | 'interpreted'>('ingestion')

function toggleDebuggerPanel() {
  isDebuggerPanelOpen.value = !isDebuggerPanelOpen.value
  
  if (isDebuggerPanelOpen.value && !ingestionStore.isListening) {
  ...
  <div class="fixed bottom-4 right-4 z-50 font-mono text-xs shadow-2xl">
    ...
    >
      <div class="flex justify-between items-center bg-zinc-800 border-b border-zinc-700">
        <div class="flex">
          <button 
            @click="selectedDebuggerTab = 'ingestion'" 
            :class="['px-4 py-2 text-[11px] font-bold uppercase tracking-wider', selectedDebuggerTab === 'ingestion' ? 'bg-zinc-700 text-white' : 'text-zinc-500 hover:text-zinc-300']"
          >
            Raw Stream
          </button>
          <button 
            @click="selectedDebuggerTab = 'interpreted'" 
            :class="['px-4 py-2 text-[11px] font-bold uppercase tracking-wider', selectedDebuggerTab === 'interpreted' ? 'bg-blue-900 text-blue-200' : 'text-zinc-500 hover:text-zinc-300']"
          >
            Interpreted State
          </button>
        </div>
        <button @click="clearEventLog" class="text-zinc-500 hover:text-white transition px-4 py-2">Clear</button>
      </div>
      
      <div v-if="selectedDebuggerTab === 'ingestion'" class="flex-1 overflow-y-auto p-4 space-y-3 bg-zinc-950/50">
